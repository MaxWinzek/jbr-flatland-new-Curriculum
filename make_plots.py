    ...
    if env == -2:
        for path in paths:
            ...
            for env in itertools.count():
                ...
                datam[:, 2] = transform(datam[:, 2])
                add_reward_trace(plot, datam, use_steps=use_steps, avg_epochs=avg_constant,
                        name=f"env_{env}")#logpath[len("logdir"):])# + path[:-len(".csv")]) # ugly yeah
        return plot


    for path in paths:
        data = load_csv(os.path.join(logpath, "plots", path))
        if env == -1 or group_mode == "global":
            data = data[:, 0:3]
        else:
            mask = (data[:, 3] == env)
            data = data[:, 0:3][mask]
...
if __name__ == "__main__":
    ...
    use_steps = True #True #(name != "percent_done")#(name == "time")
...
#python make_plots.py --logpath run7 --name percent_done --env -2
