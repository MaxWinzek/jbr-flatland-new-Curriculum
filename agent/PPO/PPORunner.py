...
class PPORunner():
    ...
    def run(self, env, controller):
        ...
        if hasattr(self.env, 'report_outcome'):
            self.env.report_outcome(percent_done)
        info = { "reward": self.env.get_total_reward(),
                 "percent_done": percent_done,
                 "steps_done": steps_done}
        if hasattr(self.env, 'get_curriculum_metrics'):
            info.update(self.env.get_curriculum_metrics())
        return self.rollout, info
